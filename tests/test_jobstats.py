...
def cpu_total_time_missing(mocker):
    ...
    def side_effect(mylist, stderr=DEVNULL):
        ...
        if mylist == ["sacct", "-V"]:
            return bytes(f"slurm {SLURM_VERSION}\n", "utf-8")
        elif mylist == ["sacct", "-P", "-X", "-o", fields, "-j", "49589697"]:
            return sacct_bytes
    mocker.patch("subprocess.check_output", side_effect=side_effect)
...
def simple_gpu_job(mocker):
    ...
    def side_effect(mylist, stderr=DEVNULL):
        ...
        if mylist == ["sacct", "-V"]:
            return bytes(f"slurm {SLURM_VERSION}\n", "utf-8")
        elif mylist == ["sacct", "-P", "-X", "-o", fields, "-j", "60155093"]:
            return sacct_bytes
    mocker.patch("subprocess.check_output", side_effect=side_effect)
...
def gpu_utilization_missing(mocker):
    ...
    def side_effect(mylist, stderr=DEVNULL):
        ...
        if mylist == ["sacct", "-V"]:
            return bytes(f"slurm {SLURM_VERSION}\n", "utf-8")
        elif mylist == ["sacct", "-P", "-X", "-o", fields, "-j", "845136"]:
            return sacct_bytes
    mocker.patch("subprocess.check_output", side_effect=side_effect)
...
def gpu_used_memory_missing(mocker):
    ...
    def side_effect(mylist, stderr=DEVNULL):
        ...
        if mylist == ["sacct", "-V"]:
            return bytes(f"slurm {SLURM_VERSION}\n", "utf-8")
        elif mylist == ["sacct", "-P", "-X", "-o", fields, "-j", "46915114"]:
            return sacct_bytes
    mocker.patch("subprocess.check_output", side_effect=side_effect)
